# LLM Wiki — Life Sciences / Biology

A personal knowledge base of life sciences papers, following [Karpathy's LLM Wiki pattern](https://gist.github.com/karpathy/1dd0294ef9567971c1e4348a90d69285):
...
| Category | Includes |
|---|---|
| `genomics` | Genome sequencing, GWAS, population genetics, variant calling, structural variants |
| `transcriptomics` | RNA-seq, single-cell RNA-seq, gene expression, splicing, long-read RNA |
| `epigenomics` | DNA methylation, chromatin accessibility (ATAC), histone modifications, 3D genome |
| `proteomics` | Protein structure, mass spectrometry, protein-protein interaction, AlphaFold |
| `neuroscience` | Brain cell types, neural circuits, brain development, neurological disease |
| `cell-biology` | Cell signaling, development, cell types, organoids, spatial transcriptomics |
| `computational` | Algorithms, bioinformatics tools, statistical methods, ML for biology |
| `overviews` | Synthesis pages spanning multiple papers |

Tip: classify by **method**, not topic. A methylation paper studying cancer → `epigenomics`, not a cancer category.
...
The most valuable pages are `wiki/overviews/` pages that synthesize across papers. When a question is answered well, save the answer:
...
- **English only** in wiki content (RAG-friendly; conversation can be Korean)
